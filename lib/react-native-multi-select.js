...
static defaultProps = {
    ...
    noItemsText: 'Nenhum item para exibir.',
  ...
  _getSelectLabel = () => {
    ...
    if (single) {
      ...
      return get(foundItem, displayKey) || selectText;
        ...
        const complemenText = selectedItems[0] === 'ALL' ? '' : `(${selectedItems.length}${selectedText})`;
    return `${selectText} ${complemenText}`;
  };
  ...
  _filterItemsPartial = searchTerm => {
    ...
    items.forEach(item => {
      const itemValue = get(item, displayKey);
      if (itemValue) {
        const normalizedItemValue = normalizeText(itemValue);
        if (regex.test(normalizedItemValue)) {
        ...
        {selector ? (
          ...
          <View>
            ...
            {!single && !hideTags && selectedItems.length ? (
              ...
              <View
                style={{
                  flexDirection: 'row',
                  flexWrap: 'wrap',
                  // height: !this.expandTags?  90: undefined,
                  // overflow: !this.expandTags?  'hidden': 'visible'
                }}
              >
            ...
            {/* <View style={{
              ...
              <TouchableOpacity onPress ={() => this._setExpandTags()}>
              <Icon name="chevron-down" size={24} color ={"#8300AF"}/>
              </TouchableOpacity>
              </View> */}
